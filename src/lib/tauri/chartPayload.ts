...
/** JSON payload for `save_workspace` / chart YAML (matches prior Svelte `chartDataToComputePayload`). */
...
/** Default “current sky” chart id (matches prior Svelte bootstrap). */
...
/** One real chart on first launch when no workspace is open (same idea as Svelte `layout.svelte.ts`). */
...
export function appChartFromNewHoroscopeInput(input: {
	...
	const name =
		...
		input.workspaceDefaults.locationName ||
		'Chart';
	...
	const locText = input.advancedMode
		? input.advancedLocation.trim()
		: input.location.trim();
